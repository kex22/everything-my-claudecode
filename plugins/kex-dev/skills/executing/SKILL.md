...
3. **读取 plan 头部的 `设计文档` 字段，逐一加载列出的设计文档作为执行上下文**（无此字段则跳过）
4. 批判性审查 plan — 发现问题先与用户讨论再开始
5. 从上次未完成的步骤继续（支持跨 session 恢复）
6. 创建 task 追踪每个步骤
...
实现中发现更好方案是正常的，但必须同步更新：
1. **更新 plan 文件**——删除废弃步骤，记录实际实现方式
2. **清理脚手架代码**——为原方案预留的字段/类型/接口若不再使用，立即删除
3. 在 commit message 中说明偏离原因
